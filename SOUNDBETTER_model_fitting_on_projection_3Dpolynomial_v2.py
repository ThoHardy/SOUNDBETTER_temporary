# -*- coding: utf-8 -*-
"""
Created on Wed Feb 22 13:21:18 2023
...
#%% Fit the 3D polynomial version of the Bifurcation model with the data, pre-selecting
# the noise mean and std before using the MLE method, and fixating threshsnr = -9.
...
for SubID in range(20) :
    ...
    guesses_per_sub[SubID] = np.array([a_guess, b_guess, c_guess, d_guess, k_guess])
    ...
    mle_model = minimize(MLE_bifurcation, 
                         np.array([a_guess, b_guess, c_guess, d_guess, k_guess]), 
    ...
    plt.savefig('Parameters_Convergence_Subject'+realSubIDs[SubID]+'_withPrior_3DPoly01')
    os.chdir(datapath)
    plt.close()
...
for SubID in range(20):
    ...
    os.chdir('C:/Users/thoma/OneDrive/Documents/Consciousness_is_Bifurcation/Model_Fit')
    model_to_save.to_csv('ModelFit_Subject'+realSubIDs[SubID]+'_withPrior_3DPoly01')
    os.chdir(datapath)
...
for SubID in range(20) : 
    ...
    a_guess, b_guess, c_guess, d_guess, k_guess = guesses_per_sub[SubID]
    guessed_high_values = []
    for snr in snrs :
    ...
    plt.legend()
    os.chdir('C:/Users/thoma/OneDrive/Documents/Consciousness_is_Bifurcation/Model_Fit')
    plt.savefig('Empirical_VS_Fit_Subject'+realSubIDs[SubID]+'_withPrior_3DPoly01')
    os.chdir(datapath)
    plt.close()