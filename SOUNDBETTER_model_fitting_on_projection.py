# -*- coding: utf-8 -*-
"""
Created on Fri Feb 17 11:24:59 2023
...
model_version = '00' # can be '00', '01', etc
...
parameters_names = ['k','threshsnr','step','L_high','k_high']
...
for SubID in range(20) :
    ...
    def pre_MLE_bifurcation(x,parameters):
        ...
        if int(model_version[1]) in [1,2,3,4]:
            k, k_high = np.abs(k), np.abs(k_high)
            if int(model_version[1])==2:
                L_high = np.abs(L_high)
    ...
    if model_version=='03':
        L_high_lb, L_high_ub = 0, L_high_guess + std_noise
    else :
        L_high_lb, L_high_ub = -inf, inf
    if model_version=='04':
        step_guess = mu_noise
        step_lb, step_ub = mu_noise, mu_noise
    else :
        step_lb, step_ub = -inf, inf
    if model_version=='05':
        threshsnr_lb, threshsnr_ub = -9, -9
    else :
        threshsnr_lb, threshsnr_ub = -inf, inf
        
    bounds = scipy.optimize.Bounds(lb=[-inf,threshsnr_lb,step_lb,L_high_lb,-inf], 
                          ub=[inf,threshsnr_ub,step_ub,L_high_ub,inf],
                          keep_feasible=False)
    ...
    plt.savefig('Parameters_Convergence_Subject'+realSubIDs[SubID]+'_withPrior'+model_version)
    os.chdir(datapath)
    plt.close()
...
for SubID in range(20):
    ...
    os.chdir('C:/Users/thoma/OneDrive/Documents/Consciousness_is_Bifurcation/Model_Fit')
    model_to_save.to_csv('ModelFit_Subject'+realSubIDs[SubID]+'_withPrior'+model_version)
    os.chdir(datapath)
...
for SubID in range(20) : 
    ...
    k, threshsnr, step, L_high, k_high = results[SubID]['x']
    if int(model_version[1])>0 :
        k_high = np.abs(k_high)
        if int(model_version[1])==2:
            L_high = np.abs(L_high)
    convergence = 'SUCCESSFULL'
    ...
    plt.legend()
    os.chdir('C:/Users/thoma/OneDrive/Documents/Consciousness_is_Bifurcation/Model_Fit')
    plt.savefig('Empirical_VS_Fit_Subject'+realSubIDs[SubID]+'_withPrior'+model_version)
    os.chdir(datapath)
    plt.close()
