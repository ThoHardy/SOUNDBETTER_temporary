# -*- coding: utf-8 -*-
"""
Created on Wed Feb 22 13:52:08 2023

@author: thoma
"""


#%% Fit the 2D polynomial version of the Bifurcation model with the data, pre-selecting 
# the noise mean and std before using the MLE method, and fixating threshsnr at -9dB.
...
for SubID in range(20) :
    ...
    threshsnr_guess = -9
    guesses_per_sub[SubID] = np.array([a_guess, b_guess, c_guess, threshsnr_guess, k_guess])
    ...
    mle_model = minimize(MLE_bifurcation, 
                         np.array([a_guess, b_guess, c_guess, threshsnr_guess, k_guess]), 
    ...
    plt.savefig('Parameters_Convergence_Subject'+realSubIDs[SubID]+'_withPrior_2DPoly00')
    os.chdir(datapath)
    plt.close()
...
for SubID in range(20):
    ...
    os.chdir('C:/Users/thoma/OneDrive/Documents/Consciousness_is_Bifurcation/Model_Fit')
    model_to_save.to_csv('ModelFit_Subject'+realSubIDs[SubID]+'_withPrior_2DPoly00')
    os.chdir(datapath)
...
for SubID in range(20) : 
    ...
    mu_noise, std_noise = noise_parameters[SubID]
    a, b, c, threshsnr, k = results[SubID]['x']
    convergence = 'SUCCESSFULL'
    ...
    a_guess, b_guess, c_guess, threshsnr_guess, k_guess = guesses_per_sub[SubID]
    guessed_high_values = []
    for snr in snrs :
    ...
    plt.legend()
    os.chdir('C:/Users/thoma/OneDrive/Documents/Consciousness_is_Bifurcation/Model_Fit')
    plt.savefig('Empirical_VS_Fit_Subject'+realSubIDs[SubID]+'_withPrior_2DPoly00')
    os.chdir(datapath)
    plt.close()