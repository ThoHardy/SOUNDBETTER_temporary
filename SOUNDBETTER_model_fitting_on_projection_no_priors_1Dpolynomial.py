# -*- coding: utf-8 -*-
"""
Created on Thu Feb 23 17:49:39 2023
...
#%% Fit the 1D polynomial version of the Bifurcation model with the data, with no bounds or pre-selection on
# the parameters
...
parameters_names = ['a','b','threshsnr','k','mu_noise','std_noise']
...
for SubID in range(20) :
    ...
    def pre_MLE_bifurcation(x,parameters):
        # extract parameters
        a, b, threshsnr, k, mu_noise, std_noise = parameters
    ...
    polynomial_values_of_x = np.array([[-13,1],[-5,1]])
    f_of_x = np.array([np.mean(empirical_distributions_500ms[SubID][-13]),
                      np.mean(empirical_distributions_500ms[SubID][-5])])
    [a_guess,b_guess] = np.matmul(np.linalg.inv(polynomial_values_of_x),f_of_x)
    ...
    guesses_per_sub[SubID] = np.array([a_guess, b_guess, threshsnr_guess, k_guess, mu_noise_guess, std_noise_guess])
    ...
    plt.savefig('Parameters_Convergence_Subject'+realSubIDs[SubID]+'_noPrior_1DPoly')
    os.chdir(datapath)
    plt.close()
...
for SubID in range(20):
    ...
    os.chdir('C:/Users/thoma/OneDrive/Documents/Consciousness_is_Bifurcation/Model_Fit')
    model_to_save.to_csv('ModelFit_Subject'+realSubIDs[SubID]+'_noPrior_1DPoly')
    os.chdir(datapath)
...
for SubID in range(20) : 
    ...
    a, b, threshsnr, k, mu_noise, std_noise = results[SubID]['x']
    convergence = 'SUCCESSFULL'
    ...
    a_guess, b_guess, threshsnr_guess, k_guess, mu_noise_guess, std_noise_guess = guesses_per_sub[SubID]
    guessed_low_values = [mu_noise_guess for i in range(100)]
    guessed_high_values = []
    for snr in snrs :
        mu_snr = a_guess*snr + b_guess
    ...
    plt.legend()
    os.chdir('C:/Users/thoma/OneDrive/Documents/Consciousness_is_Bifurcation/Model_Fit')
    plt.savefig('Empirical_VS_Fit_Subject'+realSubIDs[SubID]+'_noPrior_1DPoly')
    os.chdir(datapath)
    plt.close()