# -*- coding: utf-8 -*-
"""
Created on Mon Feb 20 11:57:02 2023
...
#%% Fit the ReLu-versin of the Bifurcation model with the data, using only MLE on all the parameters,
# with no condition on them.
...
parameters_names = ['a', 'threshsnr', 'std_noise', 'mu_noise', 'k']
...
for SubID in range(20) :
    ...
    def pre_MLE_bifurcation_ReLu(x,parameters):
        # extract parameters
        a, threshsnr, std_noise, mu_noise, k = parameters
    ...
    threshsnr_guess = -9
    a_guess = (np.mean(empirical_distributions_500ms[SubID][-5]) - np.mean(empirical_distributions_500ms[SubID][-9]))/4
    k_guess = a_guess
    
    # minimize arguments: function, intial_guess_of_parameters, method
    mle_model = minimize(MLE_bifurcation, 
                         np.array([a_guess, threshsnr_guess, std_noise_guess, mu_noise_guess, k_guess]), 
                         method='Nelder-Mead',
    ...
    plt.savefig('Parameters_Convergence_Subject'+realSubIDs[SubID]+'_noPrior_ReLu')
    os.chdir(datapath)
    plt.close()
...
for SubID in range(20):
    ...
    os.chdir('C:/Users/thoma/OneDrive/Documents/Consciousness_is_Bifurcation/Model_Fit')
    model_to_save.to_csv('ModelFit_Subject'+realSubIDs[SubID]+'_noPrior_ReLu')
    os.chdir(datapath)
...
for SubID in range(20) : 
    ...
    a, threshsnr, std_noise, mu_noise, k = results[SubID]['x']
    convergence = 'SUCCESSFULL'
    ...
    for snr in snrs :
        if snr < threshsnr :
            ...
            mu_snr = a*snr + (mu_noise - a*threshsnr)
    ...
    plt.legend()
    os.chdir('C:/Users/thoma/OneDrive/Documents/Consciousness_is_Bifurcation/Model_Fit')
    plt.savefig('Empirical_VS_Fit_Subject'+realSubIDs[SubID]+'_noPrior_ReLu')
    os.chdir(datapath)
    plt.close()
