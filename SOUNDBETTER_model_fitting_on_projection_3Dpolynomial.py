# -*- coding: utf-8 -*-
"""
Created on Wed Feb 22 11:19:56 2023
...
#%% Fit the 3D polynomial version of the Bifurcation model with the data, pre-selecting as the noise mean and std 
...
parameters_names = ['a','b','c','d','threshsnr','k']
...
for SubID in range(20) :
    ...
    def pre_MLE_bifurcation(x,parameters):
        # extract parameters
        a, b, c, d, threshsnr, k = parameters
        ...
        for ind, activity in enumerate(x):
            snr = SNRs[ind//l]
            mu_snr = a*snr**3 + b*snr**2 + c*snr + d
    ...
    polynomial_values_of_x = np.array([[(-13)**3,(-13)**2,-13,1],
                                       [(-9)**3,(-9)**2,-9,1],
                                       [(-7)**3,(-7)**2,-7,1],
                                       [(-5)**3,(-5)**2,-5,1]])
    f_of_x = np.array([np.mean(empirical_distributions_500ms[SubID][-13]), 
                      np.mean(empirical_distributions_500ms[SubID][-9]),
                      np.mean(empirical_distributions_500ms[SubID][-7]),
                      np.mean(empirical_distributions_500ms[SubID][-5])])
    [a_guess,b_guess,c_guess,d_guess] = np.matmul(np.linalg.inv(polynomial_values_of_x),f_of_x)
    ...
    threshsnr_guess = -9
    guesses_per_sub[SubID] = np.array([a_guess, b_guess, c_guess, d_guess, threshsnr_guess, k_guess])
    
    # minimize arguments: function, intial_guess_of_parameters, method
    mu_maxsnr = np.mean(list(empirical_distributions_500ms[SubID][-5]))
    inf = np.inf
    bounds = scipy.optimize.Bounds(lb=[-inf,-inf,-inf,-inf,-inf,-inf], 
                          ub=[inf,inf,inf,inf,inf,inf],
                          keep_feasible=False)
    mle_model = minimize(MLE_bifurcation, 
                         np.array([a_guess, b_guess, c_guess, d_guess, threshsnr_guess, k_guess]), 
    ...
    plt.savefig('Parameters_Convergence_Subject'+realSubIDs[SubID]+'_withPrior_3DPoly00')
    os.chdir(datapath)
    plt.close()
...
for SubID in range(20):
    ...
    os.chdir('C:/Users/thoma/OneDrive/Documents/Consciousness_is_Bifurcation/Model_Fit')
    model_to_save.to_csv('ModelFit_Subject'+realSubIDs[SubID]+'_withPrior_3DPoly00')
    os.chdir(datapath)
...
for SubID in range(20) : 
    ...
    mu_noise, std_noise = noise_parameters[SubID]
    a, b, c, d, threshsnr, k = results[SubID]['x']
    convergence = 'SUCCESSFULL'
    ...
    a_guess, b_guess, c_guess, d_guess, threshsnr_guess, k_guess = guesses_per_sub[SubID]
    guessed_high_values = []
    for snr in snrs :
        mu_snr = a_guess*snr**3 + b_guess*snr**2 + c_guess*snr + d_guess
    ...
    plt.legend()
    os.chdir('C:/Users/thoma/OneDrive/Documents/Consciousness_is_Bifurcation/Model_Fit')
    plt.savefig('Empirical_VS_Fit_Subject'+realSubIDs[SubID]+'_withPrior_3DPoly00')
    os.chdir(datapath)
    plt.close()