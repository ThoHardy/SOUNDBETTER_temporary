# -*- coding: utf-8 -*-
"""
Created on Mon Feb 20 13:52:00 2023
...
parameters_names = ['a','threshsnr','k']
...
for SubID in range(20) :
    ...
    def pre_MLE_bifurcation_ReLu(x,parameters):
        # extract parameters
        a, threshsnr, k = parameters
        ...
        for ind, activity in enumerate(x):
            ...
            if snr < threshsnr:
                ...
                mu_snr = a*(snr - threshsnr) + mu_noise
    ...
    def MLE_bifurcation(parameters):
        neg_LL = pre_MLE_bifurcation_ReLu(x,parameters)
        return(neg_LL)
    
    
    # define the intial guesses for the parameters
    threshsnr_guess = -13
    a_guess = (np.mean(empirical_distributions_500ms[SubID][-5]) - np.mean(empirical_distributions_500ms[SubID][-20]))/8
    k_guess = a_guess
    parameters_guess = np.array([a_guess,threshsnr_guess,k_guess])
    guesses_per_sub[SubID] = parameters_guess
    
    
    # minimize arguments: function, intial_guess_of_parameters, method
    mu_maxsnr = np.mean(list(empirical_distributions_500ms[SubID][-5]))
    inf = np.inf
    bounds = scipy.optimize.Bounds(lb=[-inf,-inf,-inf], 
                          ub=[inf,inf,inf],
                          keep_feasible=False)
    ...
    plt.savefig('Parameters_Convergence_Subject'+realSubIDs[SubID]+'_withPrior_ReLu')
    os.chdir(datapath)
    plt.close()
...
for SubID in range(20):
    ...
    os.chdir('C:/Users/thoma/OneDrive/Documents/Consciousness_is_Bifurcation/Model_Fit')
    model_to_save.to_csv('ModelFit_Subject'+realSubIDs[SubID]+'_withPrior_ReLu')
    os.chdir(datapath)
...
for SubID in range(20) : 
    ...
    # get guess
    a_guess, threshsnr_guess, k_guess = guesses_per_sub[SubID]
    
    # get guessed lists for the high state
    guessed_high_values = []
    for snr in snrs :
        if snr < threshsnr_guess :
            mu_snr = mu_noise
        else :
            mu_snr = a_guess*(snr - threshsnr_guess) + mu_noise
    ...
    plt.plot(snrs,guessed_high_values,color='green',label='guessed high state',linestyle='--')
    plt.legend()
    os.chdir('C:/Users/thoma/OneDrive/Documents/Consciousness_is_Bifurcation/Model_Fit')
    plt.savefig('Empirical_VS_Fit_Subject'+realSubIDs[SubID]+'_withPrior_ReLu')
    os.chdir(datapath)
    plt.close()