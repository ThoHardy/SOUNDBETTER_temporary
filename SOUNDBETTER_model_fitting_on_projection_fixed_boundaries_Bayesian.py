# -*- coding: utf-8 -*-
"""
Created on Fri Feb 24 12:26:35 2023

@author: thoma
"""


#%% Fit the Bifurcation model with the data, using Bayesian MLE on all the parameters,
# with but L_high and step are fixed on the distribution before the fitting.
...
parameters_names = ['k','threshsnr','k_high','mu_noise','std_noise']
...
for SubID in range(20) :
    ...
    L_high = np.mean(empirical_distributions_500ms[SubID][-5]) - mu_noise_guess
    k_guess, k_high_guess = L_high/8, L_high/8
    step = mu_noise_guess
    threshsnr_guess = -9
    guesses_per_sub[SubID] = [k_guess,threshsnr_guess,k_high_guess,mu_noise_guess,std_noise_guess,L_high,step]
    
    # priors on relevant parameters :
    mu_threshsnr, std_threshsnr = -9, 2
    mu_k, std_k = 0.5, 1
    mu_k_high, std_k_high = 0.5, 1
    
    
    # pre - function to minimize
    def pre_MLE_bifurcation(x,parameters):
        # extract parameters
        k, threshsnr, k_high, mu_noise, std_noise = parameters
        ...
        for ind, activity in enumerate(x):
            ...
            prior_k = np.exp(-(k-mu_k)**2/(2*std_k**2))/(std_k*np.sqrt(2*np.pi))
            prior_k_high = np.exp(-(k_high-mu_k)**2/(2*std_k_high**2))/(std_k_high*np.sqrt(2*np.pi))
            # p(x=activity|SNR=snr)
            p = beta*p_high + (1-beta)*p_low
            neg_LL -= (np.log(p) + np.log(prior_threshsnr) + np.log(prior_k) + + np.log(prior_k_high))
    ...
    mle_model = minimize(MLE_bifurcation, 
                         np.array([k_guess, threshsnr_guess, k_high_guess, mu_noise_guess, std_noise_guess]), 
                         method='Nelder-Mead',
    ...
    plt.savefig('Parameters_Convergence_Subject'+realSubIDs[SubID]+'_Bayesian_fixedBoundaries')
    os.chdir(datapath)
    plt.close()
...
for SubID in range(20):
    ...
    os.chdir('C:/Users/thoma/OneDrive/Documents/Consciousness_is_Bifurcation/Model_Fit')
    model_to_save.to_csv('ModelFit_Subject'+realSubIDs[SubID]+'_Bayesian_fixedBoundaries')
    os.chdir(datapath)
...
for SubID in range(20) : 
    ...
    k, threshsnr, k_high, mu_noise, std_noise = results[SubID]['x']
    k_guess, threshsnr_guess, k_high_guess, mu_noise_guess, std_noise_guess, L_high, step = guesses_per_sub[SubID]
    convergence = 'SUCCESSFULL'
    ...
    # get guessed lists for high and low state
    guessed_low_values = [mu_noise_guess for i in range(100)]
    guessed_high_values = []
    for snr in snrs :
        mu_snr = L_high/(1+np.exp(-k_high_guess*(snr-threshsnr_guess))) + step
    ...
    plt.legend()
    os.chdir('C:/Users/thoma/OneDrive/Documents/Consciousness_is_Bifurcation/Model_Fit')
    plt.savefig('Empirical_VS_Fit_Subject'+realSubIDs[SubID]+'_Bayesian_fixedBoundaries')
    os.chdir(datapath)
    plt.close()