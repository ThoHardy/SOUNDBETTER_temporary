# -*- coding: utf-8 -*-
"""
Created on Wed Feb 22 11:44:23 2023
...
#%% Fit the 3D polynomial version of the Bifurcation model with the data, with no bounds or pre-selection on
# the parameters
...
parameters_names = ['a','b','c','d','threshsnr','k','mu_noise','std_noise']
...
for SubID in range(20) :
    ...
    def pre_MLE_bifurcation(x,parameters):
        # extract parameters
        a, b, c, d, threshsnr, k, mu_noise, std_noise = parameters
    ...
    guesses_per_sub[SubID] = np.array([a_guess, b_guess, c_guess, d_guess, threshsnr_guess, k_guess, mu_noise_guess, std_noise_guess])
    ...
    plt.savefig('Parameters_Convergence_Subject'+realSubIDs[SubID]+'_noPrior_3DPoly')
    os.chdir(datapath)
    plt.close()
...
for SubID in range(20):
    ...
    os.chdir('C:/Users/thoma/OneDrive/Documents/Consciousness_is_Bifurcation/Model_Fit')
    model_to_save.to_csv('ModelFit_Subject'+realSubIDs[SubID]+'_noPrior_3DPoly')
    os.chdir(datapath)
...
for SubID in range(20) : 
    ...
    a, b, c, d, threshsnr, k, mu_noise, std_noise = results[SubID]['x']
    convergence = 'SUCCESSFULL'
    ...
    a_guess, b_guess, c_guess, d_guess, threshsnr_guess, k_guess, mu_noise_guess, std_noise_guess = guesses_per_sub[SubID]
    ...
    plt.legend()
    os.chdir('C:/Users/thoma/OneDrive/Documents/Consciousness_is_Bifurcation/Model_Fit')
    plt.savefig('Empirical_VS_Fit_Subject'+realSubIDs[SubID]+'_noPrior_3DPoly')
    os.chdir(datapath)
    plt.close()